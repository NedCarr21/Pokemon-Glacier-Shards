...
# pbReceivePots or pbReceivePots(qty) for more than 1 pots. (Max. 10)
################################################################################
# SETTINGS
#===============================================================================
# The count of Pots the player starts with. (Default: 4)
INITIAL_POT_COUNT = 4
# Set apricorn as a plantable berry inside Berry Pots. (Default: false)
...
class ItemBerryPots_Scene
  def pbStartScene
    ...
    @sprites["background"].setBitmap(_INTL("Graphics/UI/BerryPots/bg_pot"))
    @sprites["space"] = IconSprite.new(0,64,@viewport)
    @sprites["space"].setBitmap(_INTL("Graphics/UI/BerryPots/pot_space"))
    @spark = []
    for i in 0...potsCount
      @sprites["pot#{i}"] = IconSprite.new(4 + xPos(i),136+yPos(i),@viewport)
      @sprites["pot#{i}"].setBitmap(_INTL("Graphics/UI/BerryPots/pot"))
    ...
    @sprites["arrowu"].setBitmap(_INTL("Graphics/UI/BerryPots/arrowu"))
    @sprites["arrowu"].src_rect = Rect.new(0,0,24,22)
    @sprites["arrowu"].z = 8
    @sprites["arrowd"] = IconSprite.new(452,246,@viewport)
    @sprites["arrowd"].setBitmap(_INTL("Graphics/UI/BerryPots/arrowd"))
    ...
    @sprites["cancel"].setBitmap(_INTL("Graphics/UI/BerryPots/cancel_btn"))
    @sprites["cancel"].src_rect = Rect.new(0,0,124,48)
    @sprites["cancel"].z = 8
    
    @cursorname = "cursor-anim"
    @sprites["cursor"] = IconSprite.new(0,150,@viewport)
    @sprites["cursor"].setBitmap(_INTL("Graphics/UI/BerryPots/"+@cursorname))
    ...
    @sprites["spray"].setBitmap(_INTL("Graphics/UI/BerryPots/#{@item}"))
    ...
    @sprites["space_can"].setBitmap(_INTL("Graphics/UI/BerryPots/space_watering_can"))
  ...
  def pbUpdateCursor
    ...
    file = _INTL("Graphics/UI/BerryPots/"+curtype)
  ...
  def pbWateringAnim(i)
    ...
    loop do
      if frame == 10
        @sprites["spray"].setBitmap("Graphics/UI/BerryPots/#{@item}_anim")
      ...
      if frame == 80
        @sprites["spray"].setBitmap("Graphics/UI/BerryPots/#{@item}")
        @sprites["spray"].src_rect = Rect.new(0,0,charwidth,charheight)
      end
      # anim berry tree
  ...
  # @return [Integer] the number of pots
  def potsCount
    $PokemonGlobal.berrypot_count = INITIAL_POT_COUNT if !$PokemonGlobal.berrypot_count
    return $PokemonGlobal.berrypot_count
  end
  
  # @return [Boolean] whether the Player has mulch in bag
  def hasMulchItem
    ret = false
  ...
  # @return [Boolean] whether the Player has berry in bag
  def hasBerryItem
    ret = false
    GameData::Item.each { |i| 
  ...
  # @return [Boolean] whether the Player has apricorn in bag
  def hasApricorn
    ret = false
    GameData::Item.each { |i| 
