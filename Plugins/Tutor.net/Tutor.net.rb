...
class TutorNetPartyBlankPanel < Sprite
...
class TutorNetPartyPanel < Sprite
	attr_reader :pokemon
...
def tutor_net_build_list(commands,moveDefault,movelist,monspecificlist=nil)
	for i in 0...$player.tutorlist.length  
		if !$player.tutorlist[i].is_a?(Array)
			makeit=[$player.tutorlist[i],0]
			$player.tutorlist[i]=makeit
		end
	end   
	for i in $player.tutorlist
...
def pbMoveTutorNetChoose(commands, cmd, movelist = nil, bymachine = false, oneusemachine = false ,tutorlist=[],mode=[[],-1],monspecificlist=[])
	...
	loop do
		...
		if pokemon.egg?
			...
			if cost>0
				if currency == "$"
					purchase_msg = _INTL("Purchase this move for {1}{2}?\nYou currently have ${3}",currency,cost,$player.money)
				elsif currency == "c"
		...
		if $player.tutorlist[i][0]==move	
			...
			if ($player.tutorlist[i][1]!=cost || $player.tutorlist[i][2]!=currency) && $player.tutorlist[i][1]>0 
				if cost ==0
					...
					current_cost = ""
					new_cost = ""
					if $player.tutorlist[i][2]=="$"
						current_cost = _INTL("Current cost: ${1}",$player.tutorlist[i][1])
					else
						current_cost = _INTL("Current cost: {1} {2}",$player.tutorlist[i][1],GameData::Item.get($player.tutorlist[i][2]).name_plural)
					end	
					if currency=="$"
...
MenuHandlers.add(:pokegear_menu, :tutornet, {
		...
		"icon_name" => "tutornet",
		"order"     => 50,
		"condition" => proc { next $player.tutornet==true },
		"effect"    => proc { |menu|
			pbFadeOutIn {
				scene = PokemonTutorNet_Scene.new
				screen = PokemonTutorNetScreen.new(scene)
